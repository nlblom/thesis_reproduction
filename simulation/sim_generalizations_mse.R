##########################################################################
##### MSc Thesis - Niels Blom - Simulation Study (Chapter 3) ###
##########################################################################

# =============================================================================
# SIMULATION GRID: K (number of blocks) x rho_across (precision matrix density)
# =============================================================================
# rho_within is fixed at 0.8 throughout - realistic for SPF-style groupings.
...
#####   - MC-integrated log score: averages predictive densities  #
#####     across posterior draws, not the parameters themselves  ##
###################################################################
...
#####   Runs all five precision-estimation methods on one DGP  #####
#####   draw and returns MSE of Winkler weights + MC log score #####
#####   for each. Methods:                                     #####
...
#####                                                           #####
#####   bias_tau = 0 by default (Parts 5-7). Set > 0 only for   #####
#####   the Part 8 zero-mean robustness check.                  #####
###################################################################
...
run_one_replication <- function(p, T_obs, seed, K, rho_across, block_sizes,
  ...
  X <- sweep(X, 2, bias_vector, "+")   # no-op when bias_tau = 0
  S <- cov(X)

  # Held-out test point for the log score
  theta_test <- rnorm(1)
  u_test     <- mvrnorm(n = 1, mu = rep(0, p), Sigma = Sigma_true)
  mu_test    <- theta_test + bias_vector + u_test   # forecasts inherit the bias too
  y_test     <- theta_test            # true value to predict
  ...
  # draw M times from this conjugate posterior rather than inverting the mean once.
  # ---------------------------------------------------------------
  nu_post  <- (p + 1) + T_obs
  ...
  # MC integration: loss per draw, then averaged - averaging weights first
  # would collapse the between-draw variance in Omega.
  # ---------------------------------------------------------------
  ...
  #
  # k_init = floor(log(p)*3). joint_fit$Omega is already
  # the precision of y (Winkler's convention) - Woodbury applied internally,
  # per draw. See joint_gibbs.R's header for the Omega naming-collision note.
  #
  # k_max_bfgl is NULL (uncapped) by default, matching the thesis's reported
  # simulation results. Set it (e.g. k_max_bfgl = 10, as in empirical_spf_gdp.R)
  # to shorten BFGL's runtime for a quick check of the simulation grid without
  # a full N_SIM = 20 - see README.
  # ---------------------------------------------------------------
  tryCatch({
...
N_SIM           <- 20     # final thesis run: 20 replications per grid cell
...
#####   - Loops over (K, rho_across, p), running N_SIM replications
#####     of run_one_replication() per cell
#####   - Summarises each method/metric per cell as median + IQR
#####   - Saves the raw summary table (.rds and .csv) so the
#####     figures below can be regenerated without rerunning the
#####     grid (this takes several hours at N_SIM = 20)
###################################################################
...
for (K in K_grid) {
  for (rho_across in rho_across_grid) {
    ...
    # Incremental save after each (K, rho_across) cell, so an interruption
    # partway through this multi-hour run doesn't lose everything.
...
cat("\nRaw results saved to winkler_simulation_results.rds and .csv\n")
...
#####   Adds forecaster-specific bias to the errors. K = 3,     #####
#####   rho_across = 0.1 fixed; only p and bias_tau vary. tau=0 #####
#####   reuses the Part 6 baseline at this (K, rho_across) cell. ####
#####   Method 1 (cov(X) demeans internally) is unaffected by   #####
#####   this bias; Methods 2-5 assume a zero mean and are the   #####
#####   ones this check stresses.                               #####
###################################################################

bias_tau_grid <- c(0.2, 0.5)   # subtle vs. clearly-visible bias
N_SIM_bias    <- 20
K_bias        <- 3
rho_bias      <- 0.1
...
cat("\nBias robustness results saved to bias_robustness_results.rds and .csv\n")
