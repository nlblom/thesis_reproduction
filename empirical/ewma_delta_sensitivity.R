# Robustness check: EWMA delta sensitivity (Ch4)
#
...
# years -> quarters -> delta
delta_grid <- sapply(half_life_years_grid * 4, delta_from_half_life)

grid_table <- data.frame(
  half_life_years = half_life_years_grid,
  delta           = round(delta_grid, 4)
)
cat("EWMA delta sensitivity grid (delta_ewma = 0.97 is the Section 2.4 choice):\n")
print(grid_table)
cat("\n")

# Section 3: rolling-window evaluation (EWMA only, one R, full delta grid)

# matches the headline R in empirical_spf_gdp.R
R_sens        <- 60
n_bayes_draws <- 1000
burn_in       <- 1500
...
for (i in seq_along(test_idx)) {
  ...
  for (d in seq_along(delta_grid)) {
    ...
    tryCatch({
      ...
      # scaled by ESS
      S_ewma  <- ewma_cov(E_win, delta = delta_d, scale = ess_int)
      L       <- chol(S_ewma)
...
msfe_ratio_by_delta <- msfe_by_delta / msfe_ref
...
# delta=0.97 should match the headline (verification)
