# =============================================================================
# KMAX SENSITIVITY: does the k_max=10 cap cost BFGL predictive accuracy?
# =============================================================================
# Standalone companion to empirical_spf_gdp.R, same pattern as
# ewma_delta_sensitivity.R. k_max=10 is an a priori choice (economic
# implausibility of 35 forecasters sharing 20+ common shocks, see Ch4);
...
# =============================================================================
# PART 1: Load data (same files/scaling as empirical_spf_gdp.R)
# =============================================================================
...
# Baseline (Sample Cov. Inverse) doesn't depend on k_max - one column, computed once.
...
for (i in seq_along(test_idx)) {
  ...
  theta_hat <- -colMeans(E_win)   # forERR.csv is (actual - forecast)
  ...
  for (kx in k_max_grid) {
    ...
    tryCatch({
      set.seed(base_seed + i)   # identical RNG stream across k_max, this window only
  ...
  # Checkpoint every 10 periods (same pattern as empirical_spf_gdp.R)
  if (i %% 10 == 0) {
...
# Raw sfe_mat/ls_mat/runtime_mat/etc. are not saved separately - the
# summary CSV already has every number reported in the thesis.
